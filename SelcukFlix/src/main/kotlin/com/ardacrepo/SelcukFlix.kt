...
class SelcukFlix : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse? {
        ...
        val categories = document.select("div.section").mapNotNull { section ->
            val title = section.selectFirst("h2.section-title")?.text() ?: return@mapNotNull null
            val items = section.select("div.slider-slide").mapNotNull { slide ->
                val name = slide.selectFirst("a")?.attr("title") ?: return@mapNotNull null
                val href = fixUrlNull(slide.selectFirst("a")?.attr("href")) ?: return@mapNotNull null
                val posterUrl = fixUrlNull(slide.selectFirst("img")?.attr("data-src")) ?: fixUrlNull(slide.selectFirst("img")?.attr("src"))
    ...
    override suspend fun load(url: String): LoadResponse? {
        ...
        val scoreValue = document.selectFirst("div.post-info-imdb-rating span")?.text()?.substringBefore("(")?.trim()?.toDoubleOrNull()
    ...
    override suspend fun loadLinks(data: String, isCasting: Boolean, subtitleCallback: (SubtitleFile) -> Unit, callback: (ExtractorLink) -> Unit): Boolean {
        ...
        document.select("div.alternative-links").map { element ->
            ...
            element.select("button.alternative-link").map { button ->
                ...
                Log.d("SF", "$source » $videoID » $iframe")
